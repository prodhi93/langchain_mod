...
class DistillRefineDocumentsChain(BaseCombineDocumentsChain):
    ...
    def _distill_context(
        self, inputs, **kwargs: Any):
        """Distill/summarise preceding input for refinement"""
        distiller_tokenizer = AutoTokenizer.from_pretrained("sshleifer/distilbart-xsum-12-6")
        distiller_model = AutoModelForSeq2SeqLM.from_pretrained("sshleifer/distilbart-xsum-12-6")
        distiller_pipe = pipeline("summarization", 
                                       model=self.distiller_model, 
                                       tokenizer=self.distiller_tokenizer)
        return distiller_pipe(inputs, temperature=0, max_length=50)['summary_text']
    ...
    def combine_docs(
        ...
        for doc in docs[1:]:
            ...
            distilled_context = {self._distill_context(base_inputs)}
            res = self.refine_llm_chain.predict(callbacks=callbacks, **distilled_context)
    ...
    async def acombine_docs(
        ...
        for doc in docs[1:]:
            base_inputs = self._construct_refine_inputs(doc, res)
            inputs = self._distill_context(base_inputs)
            distilled_context = {self._distill_context(base_inputs)}
            res = await self.refine_llm_chain.predict(callbacks=callbacks, **distilled_context)
            refine_steps.append(res)
        return self._construct_result(refine_steps, res)
    ...
    def _construct_initial_inputs(
        ...
        #initial_inputs = {self._distill_context(base_inputs), **kwargs}
        initial_inputs = self._distill_context(base_inputs)
